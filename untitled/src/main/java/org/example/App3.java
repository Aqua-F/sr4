...
public class App3 extends Application {
    ...
    public void start(Stage stage) throws Exception {
        ...
        numberField.setPromptText("1-10");
        ...
        updateButton.addEventHandler(ActionEvent.ACTION, event -> {
            ...
            if (index >= 1 && index <= students.size() && newGrade >= 2 && newGrade <=5){
                students.get(index-1).setGrade(newGrade);
                tableView.refresh();
            } else if ((index < 1 || index > students.size()) && newGrade >= 2 && newGrade <=5) {
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setContentText("Нет таких строк");
                alert.show();
            }else if (index >= 1 && index <= students.size() && (newGrade < 2)) {
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setContentText("За что так с человеком то?");
                alert.show();
            }else if (index >= 1 && index <= students.size() && (newGrade > 5)) {
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setContentText("А не слишком ли?");
                alert.show();
            }else {
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setContentText("Кажется кому-то надо протрезветь");
                alert.show();
            }
        });

        HBox inputFields = new HBox(10,
                new VBox(5, numberLabel, numberField),
